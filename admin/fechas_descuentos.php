 ...
 */
    ...
    class fechas_descuentosPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`fechas_descuentos`');
            $field = new IntegerField('idFechaDescuentos', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('Fecha');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('Motivo');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('fechas_descuentosssearch', $this->dataset,
                array('idFechaDescuentos', 'Fecha', 'Motivo', 'Departamento', 'Porcentaje', 'Usuarios_idUsuarios'),
                array($this->RenderText('IdFechaDescuentos'), $this->RenderText('Fecha'), $this->RenderText('Motivo'), $this->RenderText('Departamento'), $this->RenderText('Porcentaje'), $this->RenderText('Usuarios IdUsuarios')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('fechas_descuentosasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('idFechaDescuentos', $this->RenderText('IdFechaDescuentos')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Motivo', $this->RenderText('Motivo')));
        ...
        {
            //
            // View column for idFechaDescuentos field
            //
            $column = new TextViewColumn('idFechaDescuentos', 'IdFechaDescuentos', $this->dataset);
            ...
            //
            // View column for Motivo field
            //
            $column = new TextViewColumn('Motivo', 'Motivo', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('Motivo_handler');
            
            /* <inline edit column> */
            //
            // Edit column for Motivo field
            //
            $editor = new TextEdit('motivo_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Motivo', 'Motivo', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        {
            $result = new Grid($this, $this->dataset, 'fechas_descuentosGrid');
            ...
            $column->SetInsertOperationColumn($editColumn);
            /* </inline insert column> */
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'Motivo_handler', $column);
    ...
    try
    {
        $Page = new fechas_descuentosPage("fechas_descuentos.php", "fechas_descuentos", GetCurrentUserGrantForDataSource("fechas_descuentos"), 'UTF-8');
        $Page->SetShortCaption('Fechas Descuentos');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Fechas Descuentos');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("fechas_descuentos"));
