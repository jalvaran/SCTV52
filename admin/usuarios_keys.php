 ...
 */
    ...
    class usuarios_keysPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`usuarios_keys`');
            $field = new StringField('KeyUsuario');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new IntegerField('Usuarios_idUsuarios');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $this->dataset->AddLookupField('Usuarios_idUsuarios', 'usuarios', new IntegerField('idUsuarios', null, null, true), new StringField('Login', 'Usuarios_idUsuarios_Login', 'Usuarios_idUsuarios_Login_usuarios'), 'Usuarios_idUsuarios_Login_usuarios');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('usuarios_keysssearch', $this->dataset,
                array('KeyUsuario', 'Usuarios_idUsuarios_Login'),
                array($this->RenderText('KeyUsuario'), $this->RenderText('Usuarios IdUsuarios')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('usuarios_keysasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('KeyUsuario', $this->RenderText('KeyUsuario')));
            
            $lookupDataset = new TableDataset(
                new MyConnectionFactory(),
                GetConnectionOptions(),
                '`usuarios`');
            $field = new IntegerField('idUsuarios', null, null, true);
            $field->SetIsNotNull(true);
            ...
            $lookupDataset->AddField($field, false);
            $field = new StringField('TipoUser');
            $lookupDataset->AddField($field, false);
            $field = new StringField('Email');
            $lookupDataset->AddField($field, false);
            $field = new DateTimeField('FechaCreacion');
            $field->SetIsNotNull(true);
            $lookupDataset->AddField($field, false);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('Usuarios_idUsuarios', $this->RenderText('Usuarios IdUsuarios'), $lookupDataset, 'idUsuarios', 'Login', false));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('Usuarios_idUsuarios_Login', 'Usuarios IdUsuarios', $this->dataset);
            ...
            //
            $editor = new ComboBox('usuarios_idusuarios_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new MyConnectionFactory(),
                GetConnectionOptions(),
                '`usuarios`');
            ...
            $field->SetIsNotNull(true);
            $lookupDataset->AddField($field, false);
            $lookupDataset->SetOrderBy('Login', GetOrderTypeAsSQL(otAscending));
            $editColumn = new LookUpEditColumn(
                'Usuarios IdUsuarios', 
                'Usuarios_idUsuarios', 
                $editor, 
                $this->dataset, 'idUsuarios', 'Login', $lookupDataset);
        ...
        {
            //
            // Edit column for KeyUsuario field
            //
            $editor = new TextEdit('keyusuario_edit');
            
            $editor->SetPasswordMode(true);
            $editColumn = new CustomEditColumn('KeyUsuario', 'KeyUsuario', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
        ...
        {
            //
            // View column for KeyUsuario field
            //
            $column = new TextViewColumn('KeyUsuario', 'KeyUsuario', $this->dataset);
            $column->SetOrderable(true);
        ...
        {
            $result = new Grid($this, $this->dataset, 'usuarios_keysGrid');
    ...
    try
    {
        $Page = new usuarios_keysPage("usuarios_keys.php", "usuarios_keys", GetCurrentUserGrantForDataSource("usuarios_keys"), 'UTF-8');
        $Page->SetShortCaption('Usuarios Keys');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Usuarios Keys');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("usuarios_keys"));
