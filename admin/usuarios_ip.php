 ...
 */
    ...
    class usuarios_ipPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new IntegerField('idUsuario');
            $field->SetIsNotNull(true);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('usuarios_ipssearch', $this->dataset,
                array('Direccion', 'idUsuario'),
                array($this->RenderText('Direccion'), $this->RenderText('IdUsuario')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('usuarios_ipasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('idUsuario', $this->RenderText('IdUsuario')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('idUsuario', 'IdUsuario', $this->dataset);
            $column->SetOrderable(true);
            ...
            $editColumn = new CustomEditColumn('IdUsuario', 'idUsuario', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        {
            $result = new Grid($this, $this->dataset, 'usuarios_ipGrid');
    ...
    try
    {
        $Page = new usuarios_ipPage("usuarios_ip.php", "usuarios_ip", GetCurrentUserGrantForDataSource("usuarios_ip"), 'UTF-8');
        $Page->SetShortCaption('Usuarios Ip');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Usuarios Ip');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("usuarios_ip"));
