 ...
 */
    ...
    class prod_kitsPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`prod_kits`');
            $field = new IntegerField('idKits', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('ReferenciaKit');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('ReferenciaProducto');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        ...
        public function GetPageList()
        {
            ...
            if (GetCurrentUserGrantForDataSource('prod_kits')->HasViewGrant())
                $result->AddPage(new PageLink($this->RenderText('Prod Kits'), 'prod_kits.php', $this->RenderText('Prod Kits'), $currentPageCaption == $this->RenderText('Prod Kits')));
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('prod_kitsssearch', $this->dataset,
                array('idKits', 'ReferenciaKit', 'ReferenciaProducto', 'Cantidad'),
                array($this->RenderText('IdKits'), $this->RenderText('ReferenciaKit'), $this->RenderText('ReferenciaProducto'), $this->RenderText('Cantidad')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('prod_kitsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('idKits', $this->RenderText('IdKits')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ReferenciaKit', $this->RenderText('ReferenciaKit')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ReferenciaProducto', $this->RenderText('ReferenciaProducto')));
        ...
        {
            //
            // View column for idKits field
            //
            $column = new TextViewColumn('idKits', 'IdKits', $this->dataset);
            ...
            //
            // View column for ReferenciaKit field
            //
            $column = new TextViewColumn('ReferenciaKit', 'ReferenciaKit', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('ReferenciaKit_handler');
            
            /* <inline edit column> */
            //
            // Edit column for ReferenciaKit field
            //
            $editor = new TextEdit('referenciakit_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('ReferenciaKit', 'ReferenciaKit', $editor, $this->dataset);
            ...
            //
            // View column for ReferenciaProducto field
            //
            $column = new TextViewColumn('ReferenciaProducto', 'ReferenciaProducto', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('ReferenciaProducto_handler');
            
            /* <inline edit column> */
            //
            // Edit column for ReferenciaProducto field
            //
            $editor = new TextEdit('referenciaproducto_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('ReferenciaProducto', 'ReferenciaProducto', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        {
            $result = new Grid($this, $this->dataset, 'prod_kitsGrid');
            ...
            $column->SetInsertOperationColumn($editColumn);
            /* </inline insert column> */
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'ReferenciaKit_handler', $column);
            ...
            $column->SetInsertOperationColumn($editColumn);
            /* </inline insert column> */
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'ReferenciaProducto_handler', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for ReferenciaKit field
            //
            $column = new TextViewColumn('ReferenciaKit', 'ReferenciaKit', $this->dataset);
    ...
    try
    {
        $Page = new prod_kitsPage("prod_kits.php", "prod_kits", GetCurrentUserGrantForDataSource("prod_kits"), 'UTF-8');
        $Page->SetShortCaption('Prod Kits');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Prod Kits');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("prod_kits"));
