 ...
 */
    ...
    class subcuentasPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, false);
            $field = new StringField('Cuentas_idPUC');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('subcuentasssearch', $this->dataset,
                array('PUC', 'Nombre', 'Valor', 'Cuentas_idPUC'),
                array($this->RenderText('PUC'), $this->RenderText('Nombre'), $this->RenderText('Valor'), $this->RenderText('Cuentas IdPUC')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('subcuentasasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Cuentas_idPUC', $this->RenderText('Cuentas IdPUC')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for Cuentas_idPUC field
            //
            $column = new TextViewColumn('Cuentas_idPUC', 'Cuentas IdPUC', $this->dataset);
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for Cuentas_idPUC field
            //
            $editor = new TextEdit('cuentas_idpuc_edit');
            $editor->SetSize(45);
            $editor->SetMaxLength(45);
            $editColumn = new CustomEditColumn('Cuentas IdPUC', 'Cuentas_idPUC', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        {
            $result = new Grid($this, $this->dataset, 'subcuentasGrid');
    ...
    try
    {
        $Page = new subcuentasPage("subcuentas.php", "subcuentas", GetCurrentUserGrantForDataSource("subcuentas"), 'UTF-8');
        $Page->SetShortCaption('Subcuentas');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Subcuentas');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("subcuentas"));
